...
export default function EditorChoiceSet() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const { products: allProducts } = await productService.getAll(1, 3, {
        status: "active",
        inStock: true,
      });
      ...
      const mappedProducts = allProducts.map((p: any, idx: number) => ({
        ...
        colors: p.productColors?.map((c: any) => c.name) || ["Default"],
      }));

      setProducts(mappedProducts);
      setSelectedColors(
        Object.fromEntries(mappedProducts.map((p) => [p.id, p.colors[0]])),
      );
    } catch (error) {
      console.error("Error loading editor's choice:", error);
    } finally {
      setLoading(false);
    }
  };
